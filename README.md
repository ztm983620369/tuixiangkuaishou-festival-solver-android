...
- 线程和额外内存会按手机运行时 CPU/内存能力动态开放；Festival 原生只支持 1/2/4/8 线程，APP 会自动选择不超过本机能力的最大支持值，额外内存会随当前线程数重新计算。
- Festival 可能在时间未用满时提前结束某个搜索策略；面板会显示“提前结束未解出”、实际用时和原生失败原因。
- 如果 Festival 返回 `Max nodes reached`，后台会在本机允许范围内自动提高额外内存并重试，避免节点表过小导致复杂关卡过早失败。
