...
public class myPathfinder {
    ...
    byte[][] children, //记录当前节点的子树节点方向：1--左，2--右，4--上，8--下，用位运算操作
    ...
    public myPathfinder(int row, int col) {
        ...
        parent = new byte[mMapRows][mMapCols];
        parent2 = new byte[mMapRows][mMapCols];
        children = new byte[mMapRows][mMapCols];
                                ...
                                if (flg) {  //逆推
                                    ...
                                    if (i1 < 0 || j1 < 0 || i1 >= mMapRows || j1 >= mMapCols ||
                                        ...
                                        if (isChuanYue(true, tmpLevel, i2, j2, i1, j1, i3, j3, k)) {
                                            ...
                                            tail++;
                                            pt[tail] = i << 16 | j;
                                            parent[i][j] = ( byte ) (10 * deep_Thur + k); //穿越走法（变通的方向）
        ...
        while (p <= tail) {
            for (int k = 0; 4 > k; k++) {
                ...
                if (i1 < 0 || j1 < 0 || i1 >= mMapRows || j1 >= mMapCols || i1 == nRow && j1 == nCol) {  // 界外，或遇到箱子临时位置
                    ...
                    tail++;
                    pt0[tail] = i1 << 16 | j1;   //新的足迹
                    parent2[i1][j1] = (byte) k;  //父节点到此点的方向
                    isFound = true;
                    break;
                } else if ('-' == level[i1][j1] && !mark[i1][j1]) {
                    tail++;
        ...
        try {
            ...
            for (i = 0; i < 4; i++) {
                if (mark[row + dr4[i]][col + dc4[i]] == true) {//节点被访问过
                    ...
                    children[row][col] = (byte) (children[row][col] | mByte[i]);  //增加子树
                    ...
                    if (low_tag[row + dr4[i]][col + dc4[i]] < low_tag[row][col]) { //若子节点的 low值小于其父节点的low值
                        ...
                        if (Box_Row != row || Box_Col != col) {
                            ...
                            b_count--;        //块号减一
                            children[row][col] = (byte) (children[row][col] & (~mByte[i])); //移除此“割点”及其子树
                        }
                    }
                }
            }
