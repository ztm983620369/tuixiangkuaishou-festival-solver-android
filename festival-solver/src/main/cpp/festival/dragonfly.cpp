...
#ifdef __ANDROID_FESTIVAL__
	if (cores_num == 1) dragonfly_max_nodes = 1 << 16;
	if (cores_num == 2) dragonfly_max_nodes = 1 << 17;
	if (cores_num == 4) dragonfly_max_nodes = 1 << 18;
	if (cores_num == 8) dragonfly_max_nodes = 1 << 19;
#else
...
void free_dragonfly()
{
	free(dragonfly_nodes);
	dragonfly_nodes = 0;
	dragonfly_nodes_num = 0;
	dragonfly_max_nodes = 0;
}
